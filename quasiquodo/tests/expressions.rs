...
#[test]
fn test_escape_dollar_ident() {
    let expr: Expr = ts_quote!("$$foo" as Expr);
    assert_eq!(to_code(&expr), "$foo");
}

#[test]
fn test_escape_bare_dollar() {
    let expr: Expr = ts_quote!("$$" as Expr);
    assert_eq!(to_code(&expr), "$");
}

#[test]
fn test_escape_alongside_variable() {
    // `$$prefix` is the literal identifier `$prefix`; `$val` is a variable.
    let val: Expr = ts_quote!("42" as Expr);
    let expr: Expr = ts_quote!("$$prefix + $val" as Expr, val: Expr = val);
    assert_eq!(to_code(&expr), "$prefix + 42");
}
